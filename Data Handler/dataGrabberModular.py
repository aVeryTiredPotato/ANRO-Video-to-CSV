...
# --- Runtime defaults ---
DEFAULT_FORCE_CPU = any(arg.lower() == "--force-cpu" for arg in sys.argv)
reader = None
_reader_gpu_state = None

# --- Paths ---
base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_VIDEO_NAME = "2025-11-04 17-16-32.mkv"  # Ensure that you change the name of this file
...
RAW_OUTPUT_CSV = DEFAULT_RAW_OUTPUT_CSV

# --- Sampling Control ---
# Set how many frames per second to OCR (e.g., 60, 30, 15). None means process all frames.
DEFAULT_SAMPLE_FPS = 15
SAMPLE_FPS = DEFAULT_SAMPLE_FPS

# Debug output for ROI snapshots (set False to disable extra images and conf columns)
DEFAULT_DEBUG_TOGGLE = True
DEFAULT_ROI_DEBUG_RATE = 50  # save every Nth frame
DEFAULT_ROI_DEBUG_DIR = "roi_debug"
DEBUG_TOGGLE = DEFAULT_DEBUG_TOGGLE
ROI_DEBUG_RATE = DEFAULT_ROI_DEBUG_RATE
ROI_DEBUG_DIR = DEFAULT_ROI_DEBUG_DIR

_custom_output_path = None
_custom_raw_output_path = None


def _init_reader(force_cpu: bool = False):
    """Instantiate or reuse the EasyOCR reader with the requested device."""
    global reader, _reader_gpu_state
    use_gpu = torch.cuda.is_available() and not force_cpu
    if reader is not None and _reader_gpu_state == use_gpu:
        return reader

    _reader_gpu_state = use_gpu
    reader = easyocr.Reader(['en'], gpu=use_gpu)
    print(f"EasyOCR initialized. GPU={'ON' if use_gpu else 'OFF'}")
...
def _run_pipeline():
    ...
    # --- Save Clean Data ---
    output_path = _custom_output_path or os.path.join(os.path.dirname(video_path), output_csv)
    ...
    try:
        if raw_rows:
            ...
            raw_output_path = _custom_raw_output_path or os.path.join(os.path.dirname(video_path), RAW_OUTPUT_CSV)
...
def start(
    ...
    *,
    ...
    """
    Run the OCR pipeline, optionally overriding the default IO and runtime settings.
    ...
    Args:
        ...
        debug: Toggle ROI debugging assets (snapshots + mini clip).
        roi_debug_rate: Override the snapshot frequency when debug is enabled.
        roi_debug_dir: Target directory for ROI snapshots.
        force_cpu: Force OCR to run on CPU even if CUDA is available.
    """
    global video_path, output_csv, error_log, RAW_OUTPUT_CSV
    global SAMPLE_FPS, DEBUG_TOGGLE, ROI_DEBUG_RATE, ROI_DEBUG_DIR
    ...
    ROI_DEBUG_DIR = roi_debug_dir or DEFAULT_ROI_DEBUG_DIR
    error_log = error_log_file or DEFAULT_ERROR_LOG
    ...
    if output_file:
        ...
        _custom_output_path = abs_out
        output_csv = os.path.basename(abs_out)
    else:
        output_csv = DEFAULT_OUTPUT_CSV

    if raw_output_file:
        abs_raw = os.path.abspath(raw_output_file)
        _custom_raw_output_path = abs_raw
        RAW_OUTPUT_CSV = os.path.basename(abs_raw)
    else:
        RAW_OUTPUT_CSV = DEFAULT_RAW_OUTPUT_CSV
    ...
    state_buffers = {"coolant": [], "feedwater": []}
    pending_updates = {"fuel": {"cand": None, "count": 0}}
    pending_jumps = {k: {"cand": None, "count": 0} for k in ["temperature", "pressure", "rod_insertion"]}
    conf_ema = {}

    force_cpu_resolved = DEFAULT_FORCE_CPU if force_cpu is None else force_cpu
    _init_reader(force_cpu_resolved)

    if DEBUG_TOGGLE:
        os.makedirs(ROI_DEBUG_DIR, exist_ok=True)
        clip_dir = os.path.dirname(video_path)
        clip_path = os.path.join(clip_dir, "test_clip.mp4")
        try:
            clip = mp.VideoFileClip(video_path, audio=False).subclipped(0, 60)
            clip.write_videofile(clip_path, codec="libx264", audio=False, logger='bar')
            clip.close()
            video_path = clip_path
        except Exception as clip_error:
            print(f"Warning: failed to build debug clip: {clip_error}")
